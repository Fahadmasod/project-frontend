...
const WalimaTable = () => {
  ...
  const handleAddMember = () => {
    ...
    if (newMember.group && newMember.name && !isNaN(peopleCount)) {
      fetch(`${base_url}/apii/groups/${newMember.group}/member`, {
        method: 'POST',
  ...
  const toggleCheckbox = (groupIdx, memberIdx) => {
    ...
    fetch(`${base_url}/apii/groups/${group._id}/member/${member._id}`, {
      method: 'PUT',
    })
      .then((response) => response.json())
      .then((updatedMember) => {
  ...
  const handleDeleteMember = async (groupId, memberId, groupIdx, memberIdx) => {
    try {
      ...
      const response = await fetch(`${base_url}/apii/groups/${groupId}/member/${memberId}`, {
        method: 'DELETE',
      });
